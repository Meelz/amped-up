...
public class Song_Parser
{
    ...
    public struct Metadata
    {
        ...
        public float bpm;
        public float easyDiff;
        public float medDiff;
        public float hardDiff;
            ...
            else if (line.StartsWith("#"))
            {
                ...
                switch (key.ToUpper())
                {
                    ...
                    case "MUSIC":
                        ...
                        break;
                    case "SONG":
                        songData.musicBG = fileDir + line.Substring(line.IndexOf(':')).Trim(':').Trim(';');
                        break;
                    case "INST":
                        songData.musicInst = fileDir + line.Substring(line.IndexOf(':')).Trim(':').Trim(';');
                        break;
                    case "MOVIES":
                        songData.movies = true;
                        break;
                    case "OFFSET":
                    ...
                    case "DISPLAYBPM":
                        ...
                        break;
                    case "EASYDIFF":
                        if(!float.TryParse(line.Substring(line.IndexOf(':')).Trim(':').Trim(';'), out songData.easyDiff))
                        {
                            songData.easyDiff = 0.0f;
                        }
                        break;
                    case "MEDIUMDIFF":
                        if (!float.TryParse(line.Substring(line.IndexOf(':')).Trim(':').Trim(';'), out songData.medDiff))
                        {
                            songData.easyDiff = 0.0f;
                        }
                        break;
                    case "HARDDIFF":
                        if (!float.TryParse(line.Substring(line.IndexOf(':')).Trim(':').Trim(';'), out songData.hardDiff))
                        {
                            songData.easyDiff = 0.0f;
                        }
                        break;
